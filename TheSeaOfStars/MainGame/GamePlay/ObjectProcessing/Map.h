...
public:
	void LoadMap(const char* Pathtxt, const char* PathIMG, SDL_Renderer* Game_Renderer);
};


void Map::LoadMap(const char* Pathtxt, const char* PathIMG, SDL_Renderer* Game_Renderer)
{
	FILE* filemap;
	...
	{
		this->Load_IMG(PathIMG, Game_Renderer);
		...
		while (fscanf_s(filemap, "%d ", &IDreaded) != EOF)
		{
			...
			InputPositionMaptiles = { (((Xtiles - 1) * 16) + 1), (((Ytiles - 1) * 16) + 1), 16, 16 };
